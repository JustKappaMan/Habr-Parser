...
def main():
    ...
    cli_parser.add_argument("-l", "--language", help="задаёт язык статей", choices=["ru", "en"], default="ru")

    cli_parser.add_argument("-f", "--format", help="задаёт формат вывода", choices=["json", "csv"], default="json")
    ...
    try:
        ...
        rows = [{"title": title, "author": author, "date": date} for title, author, date in zip(titles, authors, dates)]
